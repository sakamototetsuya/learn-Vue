...
# 四.使用 loader 配置处理 CSS 文件

> webpack 默认只能打包 JS 文件，无法处理其他类型的文件，为此，我们需要安装一些合适的第三方 **loader** 加载器

1.首先，加载位于 `根目录/src/css/index.css` 中的 `index.css` 文件。在 `main.js` 文件中输入（具体看自己放的位置）：

```js
import './css/index.css'
```

*运行 node ，你会发现 index.css 中的样式并没有起效，这是因为 webpack 默认只能打包 JS 文件而无法打包非JS文件*

2.为了能够打包 CSS 文件，我们需要安装 **loader** 

```cmd
npm i style-loader css-loader -D
```

上面的命令安装了两个 loader ，需要结合使用

打开 `webpack.config.js` ，新增一个配置节点 **module** 对象。其中，它有一个 **rules** 属性，它是一个数组，用于存放所有第三方文件的匹配和处理规则
...
- **module** 节点是一个**对象**用于配置所有第三方的加载器
- **rules** 是一个**数组**，用于定义所有第三方模块的加载规则
- 在 **rules** 下，以**对象**的形式配置处理非JS文件的 **loader** 规则
  1. **test** 的值是一个**正则表达式**，规定匹配对象为何
  2. **use** 的值是一个**数组**，用于存放作用于该匹配对象的 **loade** 规则

***注意： rules 的加载是从右向左逐次调用的，也就是说，上面的例子中——首先加载了 css-loader ，然后再将其传入 style-loader ，最后传入 webpack***

完成以上步骤，你会发现，CSS文件中的样式已经起效了

## （1）使用 render 渲染组件

在使用 vue 的时候，我们通常使用 `<template>`或者在 vue 实例直接创建组件，下面介绍一种新的方式——**render**

```js
render: function(createElement) {
   return createElement(【组件模版】)
}
```

- **render** 和 **components** 一样，写在 vue 实例中
- **render** 是一个函数，它接收一个参数（形参，可随意写），该参数是一个**组件模版对象**，可以将该组件替换到指定的地点（即 **el** 处）

> 注意：
>
> **componenets** 是将组件**放置**到指定地点，使用一个根元素包裹，且不会占用地点；
>
> **render** 则是将组件**替换**到指点地点，**且完全覆盖地点**

------

# 五.使用更高级的ES6语法

> 其实在之前，我们的项目中已经采用了部分ES6语法（比如 `import xxx form xxx`）
>
> 接下来我们再试试使用更高级的ES6语法（如本例的类Class）——-运行后发现，报错…

这是因为在 webpack 中，默认只能识别部分的 ES6 语法，换而言之就是有些ES6语法太高级，导致 webpack 无法编译，为了解决这个问题，我们需要引入一个新的 **loader** ：**babel** 

> 简单介绍一下：
>
> **babel** 是一个用于将高级语法（ES6）转化为低级语法的工具，使项目能够在使用ES6语法的情况下仍然能够成功运行

1. 在项目中使用以下命令安装相关的**babel loader**：
...
*注意：第二行命令后面是**数字 0** ，并且先安装第一行命令再安装第二行*

2. 在 **webapck.config.js** 文件中的 **modules** 节点下的 **rules** 添加以下规则：
...
- 这是用来匹配 **babel** 的规则
- **exclude** 是**排除**的意思，用来将（正则）匹配的文件排除在外。这是因为 **node_modules** 文件夹中存在大量的 JS 文件，但是这些文件并不需要使用 babel 处理，所以我们使用参数 **exclude** 将该文件夹排除
- 如果不排除掉它：①会因为打包过多文件导致速度太慢 ②打包过后可能导致这些包无法正常使用 

3. 在根目录中新建一个 **.babelrc** 的配置文件，该文件遵循 **JSON** 格式，所以在配置该文件时需要使用 **JSON** 规范：如不能使用注释，字符串需要用双引号括起来等。然后在该文件内根据我们第一步安装的包配置一下代码：
...
- **presets** 是**语法**，即安装的语法环境；**plugins** 是**插件**，即安装的babel相关插件
- 注意：**babel-preset** 和 **babel-plugin** 都是前缀名，所以在配置文件中不需要写